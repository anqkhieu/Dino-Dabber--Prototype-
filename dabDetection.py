...
def detectPose(frame, pose):
    ...
    if result.pose_landmarks: 
        mp_drawing.draw_landmarks(image=output_frame, landmark_list=result.pose_landmarks, connections=mp_pose.POSE_CONNECTIONS)
        for landmark in result.pose_landmarks.landmark:
            landmarks.append((int(landmark.x * width), int(landmark.y * height), int(landmark.z * depth)))
...
def classifyPose(frame, pose):
    ...
    def calculateAngle(l1, l2, l3):
        x1, y1, z1 = l1; x2, y2, z2 = l2; x3, y3, z3 = l3 
    ...
    try: 
        ...
        right_shoulder_angle = calculateAngle(landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value], landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value], landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value])
...
while cam.isOpened():
    res, frame = cam.read()
    if not res: print('Error.', res); break
    ...
    cv2.putText(output_frame, label, org=label_pos, lineType=cv2.LINE_AA, fontFace=font, fontScale = scale, color=[0,0,0], thickness = thinkness)
    cv2.putText(output_frame, label, org=label_pos, lineType=cv2.LINE_AA, fontFace=font, fontScale = scale, color=[255,255,255], thickness = thinkness - 3)
